...
$(function() {
	...
	var $allContentBoxes = $(".content-box"),
	    $allTabs = $(".tabs li a"),
	    $el, $colOne, $colTwo, $colThree, $colFour
	    hrefSelector = "",
	    speedOne = 0,
		speedTwo = 0,
		speedThree = 0,
	...
	$("#slot-machine-tabs").delegate(".tabs a", "mouseenter", function() {
		...
		if ( (!$el.hasClass("current")) && ($(":animated").length == 0 ) ) {
			...
			speedOne = Math.floor(Math.random()*100) + 500;
			speedTwo = Math.floor(Math.random()*100) + 500;
			speedThree = Math.floor(Math.random()*100) + 500;
			speedFour = Math.floor(Math.random()*100) + 500;
